...
public class Arrayausgabe_ArrayList extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Arrayausgabe_ArrayList frame = new Arrayausgabe_ArrayList();
	...
	public Arrayausgabe_ArrayList() {
		...
		btnEingabeZahl.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (i <= anzahl) {
					try {
						zahlen.add(i - 1, Integer.parseInt(tfZahl.getText()));
						i++;
						lblZahl.setText(i + ". Zahl:");
						tfZahl.setText("");
						tfZahl.requestFocus();
				...
				if (i > anzahl) {
					...
					for (int y = i - 2; y >= 0; y--) {
						lblAusgabeRueckwaerts.setText(lblAusgabeRueckwaerts.getText() + zahlen.get(y) + " ");
					}
		...
		lblAusgabeVorwaerts.setBounds(10, 131, 255, 21);
		...
		lblAusgabeRueckwaerts.setBounds(10, 162, 255, 21);
